...
## Post installation script for Exodia OS (Executes on target system to perform various operations) ##
...
_remove_unwanted_graphics_drivers() {
	...
	# Remove AMD drivers #
    if [[ -n "`lspci -k | grep 'Advanced Micro Devices'`" ]];
		
		then
        	
			amd_card=yes

    elif [[ -n "`lspci -k | grep 'AMD/ATI'`" ]];
		
		then

        	amd_card=yes

    elif [[ -n "`lspci -k | grep 'Radeon'`" ]];
...
_remove_unwanted_packages() {
    ...
    for rpkg in "${_packages_to_remove[@]}";
...
_perform_various_stuff() {
	...
	cp -rf /usr/share/grub/themes/exodia /boot/grub/themes
