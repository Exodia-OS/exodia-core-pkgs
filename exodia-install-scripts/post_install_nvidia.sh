...
## Post installation script for Exodia OS (only to detect nvidia) ##
...
_detect_nvidia_drivers() {
    ...
    if [[ -n "`lspci -k | grep -P 'VGA|3D|Display' | grep -w 'NVIDIA'`" ]];
       
        then
            
            nvidia_card=yes

        if [[ -n "`lsmod | grep -w nvidia`" ]];
        ...
        if [[ -n "`lspci -k | grep -wA2 'NVIDIA' | grep 'Kernel driver in use: nvidia'`" ]];
